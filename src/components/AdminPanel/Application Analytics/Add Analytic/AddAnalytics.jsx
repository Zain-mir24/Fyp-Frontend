...
function AddAnalytics() {
  ...
  const sendMonthly = async () => {
    try {
      const res = await url.post("/admin/Monthlydonation", {
        Month: Month,
        Donation: Donation,
      });
      if (res) {
        alert(`Monthly donation is added`);
      }
    } catch (e) {
      alert(e);
      console.log(e);
  ...
  const sendCity = async () => {
    try {
      const res = await url.post("/admin/Citydonation", {
        City: City,
        Donation: donation2,
      });
      if (res) {
  ...
  return (
    <div>
      <h1>Add data for Month donations</h1>
      <div className="row">
        ...
        <div className="col-lg-6">
          <h2>Select donation amount for that month</h2>
          <InputNumber
            formatter={(value) =>
      ...
      <div className="row">
        ...
        <div className="col-lg-6">
          <h2>Select donation amount for City</h2>
          <InputNumber
            formatter={(value) =>
              ` ${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ",")
            }
