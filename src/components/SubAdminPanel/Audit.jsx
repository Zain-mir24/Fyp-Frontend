...
function Audit() {
    ...
    try {
      ...
      for (let i = 0; i < res.data.view.length; i++) {
        for (let j = 0; j < res.data.view[i].subAdmins.length; j++) {
          if (
            user.userId === res.data.view[i].subAdmins[j]?.Sid?._id ||
            user.userId === res.data.view[i].subAdmins[j]?.Sid2?._id ||
            user.userId === res.data.view[i].subAdmins[j]?.Sid3?._id
          ) {
            console.log(res.data.view[i].fileName, "we are here");

            obj = {
              _id: res.data.view[i]._id,
              auditTeamname: res.data.view[i].auditTeamname,
              Member1: res.data.view[i]?.subAdmins[j]?.Sid?.name,
              Member2: res.data.view[i].subAdmins[j]?.Sid2?.name,
              Member3: res.data.view[i].subAdmins[j]?.Sid3?.name,
              email1: res.data.view[i].subAdmins[j]?.Sid?.email,
              email2: res.data.view[i].subAdmins[j]?.Sid2?.email,
              email3: res.data.view[i].subAdmins[j]?.Sid3?.email,
              campaignname: res.data.view[i].Cid.name,
              fileName: res.data.view[i]?.fileName,
            };
            array.push(obj);
          }
        }
  ...
  const uploadReport = async (_id) => {
    ...
    try {
      const res = await url.post("/Admin/UploadReport", formData);
      if (!res) {
  ...
  const TeamColumn = [
    ...
    },
    {
      ...
      render: (key, record) => (
        <Form.Item rules={[{ required: true, message: "Please uplaod doc" }]}>
          <Button
            onClick={() => {
